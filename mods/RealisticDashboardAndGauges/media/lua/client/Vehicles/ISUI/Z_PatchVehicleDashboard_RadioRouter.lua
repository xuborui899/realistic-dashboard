...
function ISVehicleDashboard:_yourDashPickRadioUI()
    ...
    -- B41/B42-safe identification:
    -- getType() is usually just "RadioRed"/"RadioBlack" regardless of module.
    local typ = item.getType and item:getType() or nil
    local ft  = item.getFullType and item:getFullType() or nil

    -- Premium Technologies Radio = RadioRed (can play CDs) :contentReference[oaicite:1]{index=1}
    if typ == "RadioRed" then return "premium" end
    if ft and ft:match("%.RadioRed$") then return "premium" end  -- matches Base.RadioRed or Radio.RadioRed
